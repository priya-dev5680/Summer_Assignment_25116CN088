...
int main()
{
    ...
    int* intersection_array=new int[len]; //initialising array to store intersction of the two arrays
    for(int i=0;i<len1;i++)
    {
        for(int j=0;j<len2;j++) //using nested loop to check each element of first array against every element of second array
        {
            if(arr1[i]==arr2[j])
            {
                ...
                for(int m=0;m<k;m++)
                {
                    if(intersection_array[m]==arr1[i]) //checking to avoid duplicate elements within the first array itself
                    {
                        flag=1;
                        break;
                    }
    ...
    for(int i=0;i<k;i++)
    {
        cout<<intersection_array[i]<<"    "; //printing the intersection of the two arrays
    }
    cout<<endl;
    delete[] arr1;
    delete[] arr2;
    delete[] intersection_array; //freeing up the allocated memory
    return 0;
}